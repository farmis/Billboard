...
extension View {
    public func showBillboard<V: View>(when condition: Binding<Bool>,
        ...
        self.modifier(AdvertisementModifier(showAd: condition, config: configuration, onAppear: onAppear, paywall: paywall))
    }
}
...
public struct AdvertisementModifier<V: View>: ViewModifier {
    ...
    init(showAd: Binding<Bool>, config: BillboardConfiguration, onAppear: @escaping () -> Void, paywall: @escaping () -> V) {
        self.showAd = showAd
        self.config = config
